...
class SpeculatorModelConfig(PydanticClassRegistryMixin, PretrainedConfig):
    ...
    def from_dict(
        ...
        """
        # Merge kwargs into config_dict (kwargs take precedence)
        config_dict = {**config_dict, **kwargs}

        # Handle base class with registry
        if cls.__name__ == "SpeculatorModelConfig":
            model_type = config_dict.get("speculators_model_type")
            if not model_type:
                raise ValueError(
            ...
            # Get the registered class for this model type
            registry = cls.registry
        ...
        # For subclasses, create instance with Pydantic validation
        # First, filter to only include fields defined in the Pydantic model
        ...
        for key, value in config_dict.items():
            if key in pydantic_fields:
                ...
                # Keep extra fields for PretrainedConfig compatibility
                extra_dict[key] = value

        # Create instance with Pydantic fields
        instance = cls(**pydantic_dict)

        # Set additional PretrainedConfig attributes
    ...
    def to_dict(self) -> dict[str, Any]:
        ...
        1. All Pydantic fields are included via model_dump()
        2. PretrainedConfig attributes are included
