...
class EagleSpeculatorConfig(SpeculatorModelConfig):
    ...
    def to_dict(self):
        """
        Serialize config to dictionary with minimal fields.
        ...
        for field_name in self.model_fields:
            ...
            if value is not None:
                if hasattr(value, "model_dump") and callable(value.model_dump):
                    ...
                    output[field_name] = value.dict()
                else:
                    output[field_name] = value

        essential_fields = ["_name_or_path", "model_type", "transformers_version"]
        parent_dict = super().to_dict()
        for field in essential_fields:
            if field in parent_dict:
                output[field] = parent_dict[field]

        if hasattr(self, "speculators_version") and self.speculators_version:
            output["speculators_version"] = self.speculators_version
    ...
    def from_dict(cls, config_dict, **kwargs):
        """
        Reconstruct config from dictionary, restoring proper type instances.

        :param config_dict: Dictionary containing config data
        ...
        if "speculators_config" in config_dict and isinstance(
            ...
            config_dict["speculators_config"] = SpeculatorsConfig(
                **config_dict["speculators_config"]
            )

        if "token_proposals" in config_dict and isinstance(
            config_dict["token_proposals"], dict
        ):
            config_dict["token_proposals"] = TokenProposalConfig(
                **config_dict["token_proposals"]
            )
